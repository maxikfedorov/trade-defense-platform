...
function App() {
	...
	const handleLoadLLM = async () => {
		...
		setCurrentStep(STEPS.LLM); // Переходим на шаг LLM
		setLlmLoading(true);
		setError(null);

		try {
			const result = await apiClient.analyzeFullRAG(
				tariffData.metadata.tnved_code,
			);
			setRagData(result);
		} catch (err) {
	...
	const handleExportPDF = async () => {
		try {
			await exportToPDF(
				"analysis-report",
				`анализ_${tariffData.metadata.tnved_code}.pdf`,
			);
		} catch (err) {
	...
	return (
		<ConfigProvider theme={customTheme}>
			<Layout style={{ minHeight: "100vh" }}>
					...
					>
						...
						<div
							style={{
								...
								borderRadius: 10,
								background:
									"linear-gradient(135deg, #8B9D83 0%, #A8C5A0 100%)",
								display: "flex",
								alignItems: "center",
				...
				>
					Департамент инвестиционной и промышленной политики города
					Москвы
				</Footer>
			</Layout>
